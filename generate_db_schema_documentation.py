...
INPUT_DATABASE_PASS = os.environ["INPUT_DATABASE_PASS"]
...
TBLS_YML = f"""
dsn: postgres://{INPUT_DATABASE_USER_NAME}:{INPUT_DATABASE_PASS}@{INPUT_DATABASE_HOST}:{INPUT_DATABASE_PORT}/{INPUT_DATABASE_NAME}?sslmode=disable
er:
  format: png
  distance: 2
exclude:
...
def filter_table(table):
    ...
    return re.sub(r"\(public\..*\.md\)", " ",remove_right_bracket)
        ...
        with open (PATH_TO_GITHUB_WORKSPACE + PATH_TO_GENERATED_DB_SCHEMA_FILE,"w") as generated_db_schema:
            ...
            try:
                ...
                if(len(undocumented_tables) != 0):
                    generated_db_schema.write("The following tables have not included in the documentation. Please include it.\n")
